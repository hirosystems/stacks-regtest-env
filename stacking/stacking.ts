...
const stackingInterval =
  typeof stackingIntervalEnv === "undefined"
    ? 2
    : parseInt(stackingIntervalEnv, 10);
const postTxWaitEnv = process.env.POST_TX_WAIT;
const postTxWait =
  typeof postTxWaitEnv === "undefined" ? 10 : parseInt(postTxWaitEnv, 10);
  ...
  await Promise.all(
    accountInfos.map(async (account) => {
      if (account.lockedAmount === 0n) {
        console.log(
          `Account ${account.stxAddress} is unlocked, stack-stx required`
        );
      ...
      console.log(
        `Account ${account.stxAddress} is locked for next cycle, skipping stacking`
      );
    })
  );
